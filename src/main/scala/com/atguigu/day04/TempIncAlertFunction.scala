...
object TempIncAlertFunction {
  ...
  class TempIncreaseAlertFunction extends KeyedProcessFunction[String, SensorReading, String] {
    ...
    override def processElement(value: SensorReading, ctx: KeyedProcessFunction[String, SensorReading, String]#Context, out: Collector[String]): Unit = {
      ...
      lastTemp.update(value.timepreture)

      val curTimerTs = timerTs.value()

      if (prevTemp == 0.0 || value.timepreture < prevTemp) {
        // 如果来的温度是第一条温度，或者来的温度小于最近一次温度
        // 删除报警定时器
        ctx.timerService().deleteProcessingTimeTimer(curTimerTs)
        // 清空保存定时器时间戳的状态变量, 使用clear方法
        timerTs.clear()
      } else if (value.timepreture > prevTemp && curTimerTs == 0L) {
